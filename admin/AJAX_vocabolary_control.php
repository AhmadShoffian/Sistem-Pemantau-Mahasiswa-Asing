	  ...
	  if ($query->num_rows > 0) {
	    while ($row = $query->fetch_row()) {
		  ...
		  if (isset($_GET['allowNew'])) {
			  $data[] = array('id' => 'NEW:'.$keywords, 'text' => $keywords.' &lt;'.('Add New').'&gt;');
			} else {
	  ...
	  while ($row = $query->fetch_row()) {
	  	...
	  	while ($voc_d = $voc_q->fetch_row()) {
	  		if (is_null($voc_d[2])) {
		  		...
		  		$li .= '<li><strong style="margin-right:10px;">'.($voc_d[0]).'</strong><span style="padding: 0 5px; border-radius:2px;" class="voc">'.$topic_d[0].' - '.$topic_d[1].'</span></li>';
	  		} else {
	  			$scope = $voc_d[2];
	  		}
	  	}
