...
// hanya administrator yang memiliki hak istimewa untuk mengubah grup pengguna
if ($_SESSION['uid'] != 1) {
...
if (isset($_POST['saveData'])) {
    ...
    // check form validity
    if (empty($groupName)) {
        utility::jsAlert(('Group name can\'t be empty')); //mfc
    } else {
        ...
        if (isset($_POST['updateRecordID'])) {
            ...
            if ($update) {
                ...
                utility::jsAlert(('Group Data Successfully Updated'));
                echo '<script type="text/javascript">parent.$(\'#mainContent\').simbioAJAX(parent.$.ajaxHistory[0].url);</script>';
            } else { utility::jsAlert(('Group Data FAILED to Updated. Please Contact System Administrator')."\nDEBUG : ".$sql_op->error); }
            exit();
        } else {
            /* INSERT RECORD MODE */
            // insert the data
            ...
            if ($insert) {
                ...
                utility::jsAlert(('New Group Data Successfully Saved'));
                echo '<script type="text/javascript">parent.$(\'#mainContent\').simbioAJAX(\''.$_SERVER['PHP_SELF'].'\');</script>';
            } else { utility::jsAlert(('Group Data FAILED to Save. Please Contact System Administrator')."\n".$sql_op->error); }
            exit();
        }
    }
    exit();
...
<div class="menuBoxInner userGroupIcon">
	<div class="per_title">
	  <h2><?php echo ('User Group'); ?></h2>
  </div>
	<div class="sub_section">
	  <div class="btn-group">
      <a href="<?php echo MWB; ?>system/user_group.php" class="btn btn-default"><i class="fa fa-bars"></i>&nbsp;<?php echo ('Group List'); ?></a>
      <a href="<?php echo MWB; ?>system/user_group.php?action=detail" class="btn btn-default"><i class="glyphicon glyphicon-plus"></i>&nbsp;<?php echo ('Add New Group'); ?></a>
	  </div>
    <form name="search" action="<?php echo MWB; ?>system/user_group.php" id="search" method="get" style="display: inline;"><?php echo ('Search'); ?> :
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
    ...
    $form->submit_button_attr = 'name="saveData" value="'.('Save').'" class="button"';
    ...
    /* Form Element(s) */
    // group
    $form->addTextField('text', 'groupName', ('Group Name').'*', $rec_d['group_name'], 'style="width: 60%;"');
    // hak istimewa
        // mendapatkan data akses grup
        $priv_data = array();
    ...
    $form->addAnything(('Privileges'), $priv_table);

    // edit mode messagge
    if ($form->edit_mode) {
        // print out the object
        echo '<div class="infoBox alert bg-gray">'.('You are going to edit Group data').' : <b>'.$rec_d['group_name'].'</b>  <br />'.('Last Update').$rec_d['last_update'].'</div>'; //mfc
    }
    echo $form->printOut();
} else {
    /* GROUP LIST */
