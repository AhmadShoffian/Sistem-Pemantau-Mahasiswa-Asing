...
class simbio_datagrid extends simbio_table
{
   ...
   protected $grid_append_rows = [];
  protected $callback_append_row = '';
        ...
        while ($_data = $this->grid_real_q->fetch_row()) {
            ...
            foreach ($this->modified_content as $_field_num => $_new_content) {
                ...
                if (isset($this->grid_result_rows[$_row][$_field_num])) {
                  ...
                  // append row
                  if (preg_match('@^callback\{.+\}@i', $this->callback_append_row)) {
                    $callback_append_func = str_replace(array('callback{', '}'), '', $this->callback_append_row);
                    if (function_exists($callback_append_func)) {
                      $this->grid_append_rows[$_row][] = $callback_append_func($obj_db, $_data);
                    } else {
                      $this->grid_append_rows[$_row][] = $callback_append_func;
                    }
                  } else {
                    $this->grid_append_rows[$_row][] = $this->callback_append_row;
                  }                
            }
    ...
    protected function makeOutput($int_num2show = 30)
    {
        ...
        if ($this->editable) {
            ...
            if ($this->chbox_property) {
                $_button_grp .= '<td><input type="button" onclick="chboxFormSubmit(\''.$this->table_name.'\', \''.$this->chbox_confirm_msg.'\')" value="'.($this->chbox_action_button?$this->chbox_action_button:('Delete Selected Data')).'" class="button btn '.($this->chbox_action_button?'btn-warning btn-other':'btn-danger btn-delete').'" /> '
  ...
  function modifyAppendRow($content) {
    $this->callback_append_row = $content;
  }
}
