...
class module extends simbio
{
    ...
    public function __construct() {
        $this->appended_first = '<li><a class="menu home#replaced#" href="index.php"><span>' . ('Home') . '</a></li><li><a class="menu opac" href="../index.php" title="' . ('View OPAC in New Window') . '" target="_blank"><span>' . ('OPAC') . '</span></a></li>';
        $this->appended_last = '<li><a class="menu logout" href="logout.php"><span>' . ('LOGOUT') . '</span></a></li>';
    }
    ...
    public function generateModuleMenu($obj_db)
    {
      ...
      if ($this->module_list = $this->getModuleMainMenu($obj_db)) {
          foreach ($this->module_list as $_module) {
              ...
              $_menu .= '<li><a class="menu '.$_module['name'].( (isset($_GET['mod']) && $_GET['mod']==$_module['path'])?' menuCurrent':'' ).'" title="'.$_module['desc'].'" href="index.php?mod='.$_mod_dir.'"><span>'.($_formated_module_name).'</span></a></li>';
          }
      }
