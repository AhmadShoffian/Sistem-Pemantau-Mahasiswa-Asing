...
if (isset($_POST['logMeIn'])) {
    ...
    if (!$username OR !$password) {
        utility::jsAlert(('Please supply valid username and password'));
        ...
        if ($logon->adminValid($dbs)) {
            ...
            if ($sysconf['login_message']) {
                echo 'alert(\''.('Welcome to Library Automation, ').$logon->real_name.'\');';
            }
...
if (isset($_POST['updatePassword'])) {
    ...
    if (empty($cpasswd)) {
        utility::jsAlert(('Current password can not be empty!'));
...
<div id="loginForm">
    <noscript>
        <div style="font-weight: bold; color: #FF0000;"><?php echo ('Your browser does not support Javascript or Javascript is disabled. Application won\'t run without Javascript!'); ?><div>
    </noscript>
    ...
    if (isset($_GET['update']) && !empty($_GET['update'])) { ?>
        ...
        <div class="alert alert-danger"><?php echo str_replace('{username}', $_uname, ('Hi {username}, please update your password!')) ?></div>
        <div class="heading1"><?php echo ('Current Password'); ?></div>
        <div class="login_input"><input type="password" name="currentPasswd" id="userName" class="login_input" /></div>
        <div class="heading1"><?php echo ('New Password'); ?></div>
        <div class="login_input"><input type="password" name="newPasswd" class="login_input" /></div>
        <div class="heading1"><?php echo ('Confirm New Password'); ?></div>
        <div class="login_input"><input type="password" name="newPasswd2" class="login_input" /></div>
        <div class="marginTop">
        <input type="submit" name="updatePassword" value="<?php echo ('Update'); ?>" class="loginButton" />
...
// page title
$page_title = ('Library Automation Login').' | '.$sysconf['library_name'];
...
if ($sysconf['template']['base'] == 'html') {
    ...
    require_once $sysconf['template']['dir'].'/template/login_template.inc.php';
}
exit();
